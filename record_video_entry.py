...
from recording_video import ParticipantVideoRecorder
...
async def entrypoint(ctx: agents.JobContext) -> None:
    ...
    print(f"[ENTRY] record_video_entry started. segment_seconds={segment_seconds}")

    recorders: Dict[str, ParticipantVideoRecorder] = {}

    def get_id(p: rtc.RemoteParticipant) -> str:
        return p.identity or p.sid

    async def ensure_recorder(p: rtc.RemoteParticipant) -> ParticipantVideoRecorder:
        pid = get_id(p)
        if pid not in recorders:
            recorders[pid] = ParticipantVideoRecorder(
                participant=p,
                segment_seconds=segment_seconds,
            )
            print(f"[RECORDER] Created recorder for participant={pid}")
    ...
    def _maybe_attach(pub: rtc.TrackPublication, p: rtc.RemoteParticipant) -> None:
        ...
        if pub.kind == rtc.TrackKind.KIND_VIDEO and pub.source == rtc.TrackSource.SOURCE_CAMERA and pub.track:
            ...
            async def set_audio() -> None:
                rec = await ensure_recorder(p)
                if rec.video_track is not None:
                    await rec.enable_audio(pub.track)  # type: ignore
                else:
                    print(f"[RECORDER] Audio arrived before video for {get_id(p)}; waiting for video")
